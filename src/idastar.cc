...
IDAStar::IDAStar(Heuristic h) {
    this->heuristic = h;
}

/* IDA*
 * 
 * Implements ideas form iterative deepening to A* -- instead of `depth`, we bound `f`.
 * f_limit is the next useful f-bound for the subtree considered by the call.
 * 
 * Has low space complexity but the repeated work (like IDS) can be expensive.
 * 
 * It's a tree search, i.e., duplicates are possible (no closed set). So it's more efficient 
 * when there are few duplicates.
 */
pair<optional<int>, optional<int>> IDAStar::RecursiveSearch(Node& node, int f_limit) {
	int f = node.cost + node.heuristic;
	if (f > f_limit) {
		// return <f(n), none>
		return pair<optional<int>, optional<int>>
            {f, nullopt};
	}
	...
	for (State& s : successors) {
		...
		pair<optional<int>, optional<int>>
            rs = this->RecursiveSearch(n_, f_limit);
...
optional<Search::Solution> IDAStar::run(State initial_state) {
	...
	while (f_limit < numeric_limits<int>::max()) {
		pair<optional<int>, optional<int>>
            solution = this->RecursiveSearch(this->start, f_limit);
		if (solution.second) {
            this->end_clock = chrono::steady_clock::now();
