...
void printSolution(Search::Solution solution, char *algorithm, int state_size,
                   bool record);
...
int main(int argc, char **argv) {
    ...
    if (argc <= 2) {
        cout << "Incorrect arguments. Run as:\n$ ./main "
                "-<astar/bfs/idastar/idfs/gbfs> <instance_1>, <instance_n>"
             << endl;
        cout << "Example:\n$ ./main -astar 0 6 1 7 4 2 3 8 5, 5 0 2 6 4 8 1 7 3"
             << endl;
        return 1;
    }
        ...
        if (!strcmp(algorithm, "-bfs")) {
            for (State initial_state : problems) {
                ...
                if (solution)
                    printSolution(solution.value(), algorithm,
                                  initial_state.size, record);
                else
                    printUnsolvable(algorithm, initial_state.size, record);
            }
...
void printSolution(Search::Solution solution, char *algorithm, int state_size,
    ...
    if (record) {
        ...
        outfile << solution.expanded_nodes << ","
                << solution.optimal_solution_length << "," << fixed
