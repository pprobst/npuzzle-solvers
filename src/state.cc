...
/*
 * Each tile in a state takes 4 bits, so the whole state has, at most, the size of 
 * 64 bits in the 15 puzzle, and 36 bits in the 8 puzzle. Because of this, the state
 * will be stored in an uint64_t (8 bytes).
 *
 * <<   shift left
 * >>   shift right
 * |    bitwise or
...
State::State(vector<uint8_t>& problem, Action action) {
    ...
    for (int i = problem.size()-1; i >= 0; i--) {
        if (problem[i] == 0) this->blank_idx = i; 
        this->id <<= 4;
        this->id |= problem[i];
    }
    ...
    if (this->size == 9) {
        ...
        this->goal_id = 18364758544493064720U; // 0xfedcba9876543210 -> goal state
    }
}
...
State State::getStateFrom(Action action) const {
    ...
    pair<uint8_t, uint8_t> new_blank_pos = this->idxPos(this->blank_idx) + MOVES.at(action);
    size_t new_blank_idx = this->posIdx(new_blank_pos);

    // Example:
    // 
    // state: 0x518630742
    //  0     0    0   0    0    0     0    5   1    8    6    3    0    7    4    2
    // 0000 0000 0000 0000 0000 0000 0000 0101 0001 1000 0110 0011 0000 0111 0100 0010
    //
    // Action::RIGHT (0 swaps with 3)
    //
    // value_to_swap: 0x30000 (3) 
    // 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0011 0000 0000 0000 0000
    //
    // blank_swap_state: 0x518600742
    // 0000 0000 0000 0000 0000 0000 0000 0101 0001 1000 0110 0000 0000 0111 0100 0010
    //
    // new_blank_idx > current_blank_idx, so: 
    // value_to_swap >>= 4 * (new_blank_idx - this->blank_idx);
    // 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0011 0000 0000 0000
    //
    ...
    // 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0011 0000 0000 0000
    // -------------------------------------------------------------------------------
    // 0000 0000 0000 0000 0000 0000 0000 0101 0001 1000 0110 0000 0011 0111 0100 0010
    ...
    if (new_blank_idx > this->blank_idx) value_to_swap >>= moves;
    else value_to_swap <<= moves;

    uint64_t new_state_id = blank_swap_state | value_to_swap;

    return State(new_state_id, action, new_blank_idx, this->size, this->dim, this->goal_id);
...
bool State::validAction(Action action, pair<uint8_t, uint8_t> pos) const {
    switch(action) {
        case Action::DOWN:
            if (pos.second == this->dim-1) return false;
            break;
        case Action::LEFT:
            if (pos.first == 0) return false;
            break;
        case Action::RIGHT:
            if (pos.first == this->dim-1) return false;
            break;
        case Action::UP:
