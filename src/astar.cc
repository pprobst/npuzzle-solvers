...
AStar::AStar(Heuristic h) { this->heuristic = h; }
 ...
 *
 * While the open list (a min-heap ordered by f, h, idx; gets the lowest value)
 * is not empty, expand a node. If the state of the curr node is not in the
 * closed set, add the state to it, and generate the successors of the current
 * node and add them to the open list.
 */
