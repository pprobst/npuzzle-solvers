...
Search::Solution Search::makeSolution(uint32_t solution_length) {
    return Solution {
        ...
        std::chrono::duration_cast<std::chrono::nanoseconds>(this->end_clock - this->start_clock).count() / 1000000000.0,
        this->heuristic.accumulator == 0 ? 0 : double(this->heuristic.accumulator) / this->heuristic.called_count, // heuristic mean value 
