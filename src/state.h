...
class State {
  public:
    ...
    inline uint8_t getTileAt(uint8_t idx) const {
        uint64_t mask = 0xf; // 0000 ... 0000 1111 --> takes the idx of the
                             // first tile (but last internally).
        mask <<= 4 * idx;    // shift left to the index position needed.
        uint8_t tile_at_idx = (this->id & mask) >> 4 * idx;
        return tile_at_idx;
    }

    inline bool isGoal() const { return this->id == this->goal_id; }
...
operator+(const std::pair<uint8_t, uint8_t> &lhs,
